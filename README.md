...
- Select `Test flight` in the top bar to fly over `Mountain_Valley_Colorado.glb`. Use arrow keys for pitch/roll, `Q` / `E` for yaw, `W` / `S` for dry throttle, hold `Shift` for afterburner, `Space` for the air brake, `F` for flaps, and `R` to reset. Digital axes ramp smoothly instead of snapping. There is no post-stall mode and no entry window: control authority and the AoA fence both follow dynamic pressure, so the slower the air the more the airframe will let a committed pull ask for. Centred stick asks the FCC to recover.
