...
- Select `Test flight` in the top bar to fly over `Mountain_Valley_Colorado.glb`. Move the mouse around the flight canvas for analogue pitch/roll, use arrow keys for digital pitch/roll, and use `Q` / `E` for yaw. The canvas is a virtual stick with a quiet centre and a progressive response toward its edges; right-drag temporarily suspends it for free look, while left click remains available to weapons. `W` and `S` are a speed control rather than a power lever: they raise and lower the commanded airspeed in km/h, the command holds where it is left, and the throttle the engine receives is derived from it. `S` also progressively opens the air brake, reaching the full high-G brake during a committed pull. The HUD's selector names the same number, and the deck shows the commanded speed next to the thrust it is costing. It is feedforward, not a speed hold — a hard turn, an open brake, or a climb bleeds the aircraft below its commanded number and it has to be flown back up to it, which is the whole energy game. Reheat is a separate burst past the top of the band rather than another position on it. Hold `Shift` for afterburner, use `F` for flaps, and press `R` to reset. Press `C` to switch between Chase and Nose cameras, right-drag to free-look around the aircraft on global world axes, and hold `V` for a rear view that keeps the aircraft in frame. Free view is a rotation of the chase pose rather than a second rig: zero drag reproduces the ordinary camera exactly, so grabbing and releasing the mouse cannot pop. The drag is inverted on both axes and orbits the aircraft's own middle at a fixed radius; letting go springs the camera home in about half a second. In the nose camera the same drag turns the pilot's head instead of moving the seat. `Esc` (or `P`, which fullscreen does not swallow) stops the simulation and opens the pause menu — resume, settings, credits, or back to the hangar; its Settings pane remembers Combat Chase or Action camera behavior and Near, Normal, or Far chase distance. Combat Chase uses a sprung 70° flight-frame view that blends nose and velocity direction. Action uses a tighter 69° lens and a closer boom, captures the entry shot when controlled PSM begins, and keeps that world-space angle after Pitch or Alt is released, so a Cobra can remain upright in frame or a flip can stop near 180 degrees. Pressing `W` is the flight-control handoff that carries the camera around the aircraft on a smooth orbit before it settles behind the tail. Digital axes ramp smoothly instead of snapping. Two keys sit above the stick and they are deliberately different regimes. `Space` — or `W` + `S` held together, which is the same command — is the high-G turn: the FCC is allowed a faster pitch and roll rate, a higher G ceiling and an alpha fence that widens to just short of the stall, and the turn is billed for all of it in induced drag, so the rate and the radius are bought with energy. The wing keeps flying throughout: the nose stays near the flight path, the trigger is worth less the slower the aircraft is, and no amount of holding it opens the post-stall envelope. Held together, `W` and `S` neither accelerate nor decelerate — the commanded speed holds and the air brake stays shut, so the chord costs exactly what `Space` costs. Without either key the FCC fades pull authority at the protected AoA fence and adds a small restoring command if momentum carries the wing toward a stall. For arcade PSM, slow below the aircraft's entry limit, hold `Left Alt`, then pull with the mouse or `Arrow Up`. Release Pitch Up to hold the attitude, keep holding Pitch Up to continue through a full flip, add `Q` or `E` for a J-turn, stop near 180° and use `W` + `Shift` for an inertial reversal, or command Pitch Down to recover. No timer ends PSM on the player's behalf. Attitude and velocity remain separate throughout, so the nose can point upward or backward while momentum continues along the original flight path.
